...
namespace PlacesIR.GooglePlaces
{
    public class GooglePlacesClient : IDisposable
    {
        ...
        private ValidationResponse<List<Place>> EvaluateResponse(ref ValidationResponse<List<Place>> response, ValidationResponse<GooglePlacesApiResponse<List<Place>>> res)
        {
            if (response==null)
            {
        ...
        private ValidationResponse<Place> EvaluateResponse(ref ValidationResponse<Place> response, ValidationResponse<GooglePlacesApiResponseSinlgeResult<Place>> res)
        {
            if (response == null)
            {
                response = new ValidationResponse<Place>();
            }
            if (res.Obj != null && res.Obj.status == "OK" && res.Obj != null)
            {
                response.Obj = res.Obj.result;
            }
            else
            {
                if (res.Errors != null)
        ...
        public ValidationResponse<List<Place>> GetNearByPlaces(ReqNearByPlaces request, int limit = 40)
        {
            ...
            #region Retrive data
            request.key = GoogleApiKey;
            ValidationResponse<GooglePlacesApiResponse<List<Place>>> res = null;
        ...
        public ValidationResponse<Place> GetPlacesDetails(ReqPlaceDetails request)
        {
            ValidationResponse<Place> validationResponse = CreateValidationResponse<Place>();
            #region Validation
